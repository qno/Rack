...
def create_plugin(slug, plugin_dir=None):
   ...
   cmakelistsfile = """cmake_minimum_required(VERSION 3.5)
...
# If you use submodules in your plugin, comment out the next lines. If not, remove the commented lines.
...
# If you want to references binary files compiled into the plugin
...
#add_library(${{LIB_NAME}} MODULE ${{SOURCES}} ${{YOUR_BINARY_ASM}})
...
[imports]
"""
   with open(os.path.join(plugin_dir, "conanfile.txt"), "w") as f:
      f.write(conanfile)
